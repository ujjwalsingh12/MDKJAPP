...
def run_free_query(
    ...
    if not conn:
        return {"status": "error", "message": "Failed to get a database connection."}
    base_query = raw_query.strip().rstrip(';')
...
import logging
def unified_update_journal_entry(
    entry_type_or_data,
    ...
    """
    Python wrapper to call the PostgreSQL unified_update_journal_entry function.
    
    Returns:
        Dict with status and journal update metadata.
    """
    ...
    try:
        if isinstance(entry_type_or_data, dict):
            ...
            igst = data.get("igst")
            weight = data.get("weight")
            cash_amount = data.get("cash_amount", data.get("amount"))
        else:
            entry_type = entry_type_or_data
        ...
        query = text("""
            SELECT * FROM public.unified_update_journal_entry(
                p_entry_type := :entry_type,
                p_entry_id := :entry_id,
                p_gstin := :gstin,
                p_is_debit := :is_debit,
        ...
        query_log = f"""
            SELECT * FROM public.unified_update_journal_entry(
                p_entry_type := {safe_format(entry_type)},
                p_entry_id := {safe_format(entry_id)},
                p_gstin := {safe_format(gstin)},
                p_is_debit := {safe_format(is_debit)},
                p_dated := {safe_format(dated)},
                p_bank := {safe_format(bank)},
                p_remark_text := {safe_format(remark_text)},
                p_bill_no := {safe_format(bill_no)},
        ...
        logging.info(f"Executing update query: {query_log}")
        result = conn.execute(query, {
            "entry_type": entry_type,
            "entry_id": entry_id,
            "gstin": gstin,
        ...
        row = result.fetchone()
        logging.info(f"Update query executed successfully, fetched row: {row}")
        ...
        if row:
            ...
            trans.rollback()
            return {"status": "error", "message": "No row returned by update"}
        ...
        logging.error(f"Error during unified update: {e}", exc_info=True)
        return {
            "status": "error",
...
def batch_insert_journal_entries(entry_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Batch inserts multiple financial entries using the unified_insert_journal_entry function.
    If any insertion fails, the entire transaction is rolled back.

    Args:
        entry_list: A list of dictionaries, each representing an entry to insert.

    Returns:
        A dictionary containing:
            - "status": "success" or "error"
            - "results": list of individual insert results if success
            - "failed_index": index of failed entry (if any)
    ...
    try:
        for i, entry in enumerate(entry_list):
            ...
            dated = entry.get("dated")
            if isinstance(dated, str):
                from datetime import datetime
                entry["dated"] = datetime.strptime(dated, '%Y-%m-%d').date()

            # Create query for unified_insert_journal_entry
            query = text("""
                SELECT * FROM public.unified_insert_journal_entry(
                    p_entry_type := :entry_type,
                    p_gstin := :gstin,
            ...
            result = conn.execute(query, {
                ...
                'is_debit': entry.get('is_debit'),
                'dated': entry.get('dated', date.today()),
                'bank': entry.get('is_bank', entry.get('bank', False)),
                'remark_text': entry.get('remark_text'),
                ...
                'cash_amount': entry.get('cash_amount', entry.get('amount'))
            })
            ...
            if not row:
                ...
                return {
                    "status": "error",
                    "message": "No result returned from unified_insert_journal_entry.",
                    "failed_index": i
                }
        ...
        logging.error(f"Batch insert failed at entry index {i}: {str(e)}", exc_info=True)
        return {
            "status": "error",
            "message": str(e),
            "error_type": type(e).__name__,
...
def unified_delete_journal_entry(
    entry_type_or_data,
    entry_id: Optional[int] = None,
    remark_text: Optional[str] = None
) -> Dict[str, Any]:
    """
    Python wrapper to call the PostgreSQL unified_delete_journal_entry function.
    
    Args:
        entry_type_or_data (str or dict): The entry type string or full data dictionary.
        entry_id (int): ID of the entry to delete.
        remark_text (str): Optional remark text for audit.

    Returns:
        Dict with status and journal metadata for deletion.
    """
    ...
    try:
        ...
        query = text("""
            SELECT * FROM public.unified_delete_journal_entry(
                p_entry_type := :entry_type,
                p_entry_id := :entry_id,
                p_remark_text := :remark_text
            )
        ...
        """
        logging.info(f"Executing delete query: {query_log}")
        ...
        row = result.fetchone()
        logging.info(f"Delete query executed successfully, fetched row: {row}")
        ...
        if row:
            ...
            trans.rollback()
            return {"status": "error", "message": "No row returned by delete"}

    except Exception as e:
        try:
            trans.rollback()
        except Exception as rollback_err:
            logging.error("Failed to rollback transaction", exc_info=True)

        logging.error(f"Error during unified delete: {e}", exc_info=True)
        return {
            "status": "error",
            "message": f"Database error: {str(e)}",
            "error_type": type(e).__name__
